...
namespace sdds {
	...
	AirportLog::AirportLog(Airport* airport, size_t size) : m_size(size)
	{
		if (airport)
			m_airportArr = airport;
	}
	...
	AirportLog AirportLog::findAirport(const char* state, const char* country) const
	{
		...
		tempAirport = new Airport[foundCount];
		for (size_t i = 0; i < foundCount; i++)
			if (m_airportArr[i].getState() == state && m_airportArr[i].getCountry() == country)
				tempAirport[j] = m_airportArr[i];

		return AirportLog(tempAirport, foundCount);
	}
